...
optimizer = torch.optim.Adam(model.parameters(), lr=4e-5156556275345)
model.load_state_dict(torch.load(r"D:\Pyprograms\New\checkpoint\2024-12-25_22-53-46\model_weights.pth"))
...
for epoch in range(10):
    for batched_graph, labels, conformer in tqdm(data_loader, desc="Training", leave=True) :        # batched_graph是dgl图 ；label 是分子性质，conformer是分子构象
        ...
        loss = criterion(out, torch.stack(input["labels"]).to(device))
        loss.backward()
        print(loss.item())
    ...
    # 保存模型的权重
    model_save_path = os.path.join(checkpoint_dir, "model_weights.pth")
