...
node_encodding=torch.rand(1,583,9)
adjacency=torch.rand(1,583,583)
conformer=torch.rand(1,583,3)
node_mask=torch.ones(1,583,1)

# 输入节点编码和每个节点的位置，输出更新后的节点特征
...
class TensorNet(nn.Module):
    ...
    def __init__(
        ...
        self.mlp1=nn.Linear(hidden_channels,512)
        self.mlp2=nn.Linear(512,128,bias=True)
        self.mlp3=nn.Linear(128,19,bias=True)
        if num_layers != 0:
    ...
    def compute_distance(self,pos,batch,edge_index):
        # 1. 计算每条边的向量表示 edge_vec[edge, 3]
        # 使用节点坐标计算边的向量
        src, dst = edge_index  # 获取边的起点和终点索引
        edge_vec = pos[dst] - pos[src]  # 每条边的向量表示

        # 2. 计算每条边的权重 edge_weight[edge, 1]（即边的欧几里得距离）
        edge_weight = torch.norm(edge_vec, dim=1, keepdim=True)  # 计算每条边的欧几里得距离
        return edge_weight,edge_vec


    def forward(
        self,
        z: Tensor,  # 原子特征(natom,dim)
        pos: Tensor,    # 位置[natom,3]
        batch: Tensor,  # 指示每个原子属于哪个原子[n,1]
        adjacency: Tensor, # 邻接矩阵[2,n]
        box: Optional[Tensor] = None,
        q: Optional[Tensor] = None,
        s: Optional[Tensor] = None,
    ) -> Tuple[Tensor, Optional[Tensor], Tensor, Tensor, Tensor]:
        # Obtain graph, with distances and relative position vectors
        # 这里要计算edge_index,edge_weight,edge_vec
        # 检查 pos 的维度并根据情况调整
        if pos.dim() == 3 and pos.shape[0] == 1:  # 如果是[1, n, 3]
            pos = pos.squeeze(0)  # 去掉第一个维度，变成[n, 3]
        if z.dim() == 3 and z.shape[0] == 1:
            z = z.squeeze(0)
        edge_index = adjacency

        edge_weight,edge_vec = self.compute_distance(pos, batch,edge_index)
        ...
        edge_attr=edge_attr.squeeze(dim=1)
        ...
        # print(edge_weight.masked_fill(mask, 1).shape)
        edge_vec = edge_vec / edge_weight
        ...
        if self.static_shapes:
            x = x[:-1]
        num_molecules =batch.max() + 1  # 3个分子

        # 聚合每个分子的原子特征
        molecule_features = torch.zeros(num_molecules, 256).to("cuda")
        out=self.mlp3(self.mlp2(self.mlp1(molecule_features)))
        return x,z, pos, batch,out
...
class TensorEmbedding(nn.Module):
    ...
    def _get_tensor_messages(
        ...
        # print(self.cutoff(edge_weight).shape)
        C = self.cutoff(edge_weight).reshape(-1, 1, 1, 1) * Zij # 计算截止权重
        eye = torch.eye(3, 3, device=edge_vec_norm.device, dtype=edge_vec_norm.dtype)[
            None, None, ...
        ]# 创建单位矩阵
        # 这里得到的I的维度应该是n,128, 3,3而不应该是n,n,128,3,3,问题出在edge_attr应该是n,128而不是n,1,128,所以添加了下面一行

        Iij = self.distance_proj1(edge_attr)[..., None, None] * C * eye # 根据单位矩阵计算I
        Aij = (
    ...
    def forward(
        ...
        if z.dtype != torch.long and z.dtype != torch.int64:
            z=z.long()
        # Zij是每条边对应的特征
        Zij = self._get_atomic_number_message(z[:,0:1], edge_index)
